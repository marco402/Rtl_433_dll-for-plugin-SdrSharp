...
float magnitude_true_cs16(int16_t const *iq_buf, uint16_t *y_buf, uint32_t len)
{
    ...
    for (i = 0; i < len; i++) {
        ...
        y_buf[i]  = (int32_t)sqrt(x * x + y * y) >> 1; // max 46341, scaled 23170, fs 16384
        sum += y_buf[i];
    }
    return len > 0 && sum >= len ? MAG_TO_DB((float)sum / len) : MAG_TO_DB(1);
}
...
#define FIX(x) ((int32_t)(x * S_CONST))
...
__declspec(dllexport) void __stdcall  baseband_low_pass_filter(filter_state_t *state, uint16_t const *x_buf, int16_t *y_buf, uint32_t len)
{
    ///  [b,a] = butter(1, 0.01) -> 3x tau (95%) ~100 samples
    //static int32_t const a[FILTER_ORDER + 1] = {FIX(1.00000) >> 1, FIX(0.96907) >> 1};
    //static int32_t const b[FILTER_ORDER + 1] = {FIX(0.015466) >> 1, FIX(0.015466) >> 1};
    ///  [b,a] = butter(1, 0.05) -> 3x tau (95%) ~20 samples
    static int32_t const a[FILTER_ORDER + 1] = {FIX(1.00000) >> 1, FIX(0.85408) >> 1};
    static int32_t const b[FILTER_ORDER + 1] = {FIX(0.07296) >> 1, FIX(0.07296) >> 1};
    // note that coeffs are prescaled by div 2
    state->memoy[0] = state->y[0];
    state->memox[0] = state->x[0];
...
static int16_t atan2_int16(int32_t y, int32_t x)
{
    ...
    if (!x && !y)
        return 0; // We would get 8191 with the code below

    if (x >= 0) { // Quadrant I and IV
        int32_t denom = (abs_y + x);
        if (denom == 0)
            denom = 1; // Prevent divide by zero
    ...
    if (y < 0)
        angle = -angle; // Negate if in III or IV
    return angle;
}
...
#define FIX32(x) ((int32_t)(x * S_CONST32))
...
void baseband_demod_FM_cs16(demodfm_state_t *state, int16_t const *x_buf, int16_t *y_buf, uint64_t num_samples, uint32_t samp_rate, float low_pass)
{
    ...
    for (uint32_t n = 0; n < num_samples; n++) {
        ...
        x0f = atan2_int32((int32_t)pi, (int32_t)pr); // Integer implementation
        // xlp = atan2_int16(pi >> 16, pr >> 16) << 16; // Integer implementation, truncated
        // xlp = pi; // Cheat and use only imaginary part (works OK, but is amplitude sensitive)
        // Low pass filter
        // y0f      = (alp[1] * y1f + blp[0] * x0f + blp[1] * x1f) >> F_SCALE32;
        y0f      =(int32_t) ((alp[1] * y1f + blp[0] * ((int64_t)x0f + x1f)) >> F_SCALE32); // note: blp[0]==blp[1]
