...
static int32_t bresser_6in1_decode(r_device *decoder, bitbuffer_t *bitbuffer, int32_t startPulses, uint16_t package_type)
{
    uint8_t const preamble_pattern[] = {0xaa, 0xaa, 0x2d, 0xd4};

    int32_t const moisture_map[] = {0, 7, 13, 20, 27, 33, 40, 47, 53, 60, 67, 73, 80, 87, 93, 99}; // scale is 20/3
    ...
    uint32_t const start_pos = bitbuffer_search(bitbuffer, 0, 0,
    ...
    uint32_t const len = bitbuffer->bits_per_row[0] - start_pos;
    if (len < sizeof(msg) * 8) {
    ...
    int32_t const chkdgst = (msg[0] << 8) | msg[1];
    int32_t const digest  = lfsr_digest16(&msg[2], 15, 0x8810, 0x5412);
    if (chkdgst != digest) {
        decoder_logf(decoder, 2, __func__, "Digest check failed %04x vs %04x", chkdgst, digest);
        return DECODE_FAIL_MIC;
    }
    // Checksum, add with carry
    int32_t const chksum = msg[17];
    int32_t const sum    = add_bytes(&msg[2], 16); // msg[2] to msg[17]
    if ((sum & 0xff) != 0xff) {
    ...
    int32_t const s_type  = (msg[6] >> 4); // 1: weather station, 2: indoor?, 3: pool thermometer, 4: soil probe
    int32_t const startup = (msg[6] >> 3) & 1; // s.a. #1214
    int32_t const chan    = (msg[6] & 0x7);
    int32_t const battery = (msg[13] >> 1) & 1; // b[13] & 0x02 is battery_good, s.a. #1993

    // temperature, humidity, shared with rain counter, only if valid BCD digits
    int32_t const temp_ok   = msg[12] <= 0x99 && (msg[13] & 0xf0) <= 0x90;
    int32_t const temp_raw  = (msg[12] >> 4) * 100 + (msg[12] & 0x0f) * 10 + (msg[13] >> 4);
    int32_t const temp_sign = (msg[13] >> 3) & 1;
    ...
    int32_t const humidity = (msg[14] >> 4) * 10 + (msg[14] & 0x0f);

    // apparently ff01 or 0000 if not available, ???0 if valid inverted BCD
    int32_t uv_ok  = (msg[16] & 0x0f) == 0 && (~msg[15] & 0xff) <= 0x99 && (~msg[16] & 0xf0) <= 0x90;
    int32_t const uv_raw = ((~msg[15] & 0xf0) >> 4) * 100 + (~msg[15] & 0x0f) * 10 + ((~msg[16] & 0xf0) >> 4);
    float const uvi   = uv_raw * 0.1f;
    int32_t const flags  = (msg[16] & 0x0f); // looks like some flags, not sure

    //int32_t const unk_ok  = (msg[16] & 0xf0) == 0xf0;
    //int32_t const unk_raw = ((msg[15] & 0xf0) >> 4) * 10 + (msg[15] & 0x0f);

    // invert 3 bytes wind speeds
    msg[7] ^= 0xff;
    msg[8] ^= 0xff;
    msg[9] ^= 0xff;
    int32_t wind_ok = (msg[7] <= 0x99) && (msg[8] <= 0x99) && (msg[9] <= 0x99);

    int32_t const gust_raw    = (msg[7] >> 4) * 100 + (msg[7] & 0x0f) * 10 + (msg[8] >> 4);
    float const wind_gust = gust_raw * 0.1f;
    int32_t const wavg_raw    = (msg[9] >> 4) * 100 + (msg[9] & 0x0f) * 10 + (msg[8] & 0x0f);
    float const wind_avg  = wavg_raw * 0.1f;
    int32_t const wind_dir    = ((msg[10] & 0xf0) >> 4) * 100 + (msg[10] & 0x0f) * 10 + ((msg[11] & 0xf0) >> 4);

    // rain counter, inverted 3 bytes BCD, shared with temp/hum, only if valid digits
    msg[12] ^= 0xff;
    msg[13] ^= 0xff;
    msg[14] ^= 0xff;
    int32_t const rain_ok   = msg[16] & 1;
    //int32_t const rain_ok   = msg[12] <= 0x99 && msg[13] <= 0x99 && msg[14] <= 0x99;
    int32_t const rain_raw  = (msg[12] >> 4) * 100000 + (msg[12] & 0x0f) * 10000
