...
as a float with 2 significant digits.  The 16 bit int32_t represents this voltage,
...
static int32_t bm5_decode(r_device *decoder, bitbuffer_t *bitbuffer, int32_t startPulses, uint16_t package_type)
{
    data_t *data;
    uint8_t b[11];
    ...
    if ((uint8_t)add_bytes(&b[0], 10) != b[10]) {
        return DECODE_FAIL_MIC; // failed checksum - invalid message
    }

    int32_t id             = (b[0] << 16) | (b[1] << 8) | b[2];
    int32_t soh            = b[3] >> 1;          // State of Health encoded in 1st 7 bits
    int32_t charging_error = b[3] & 0x01;        // Charging error flag in bit 8 of byte 4
    int32_t soc            = b[4] >> 1;          // State pf Charge encoded in 1st 7 bits
    int32_t cranking_error = b[4] & 0x01;        // Cranking error flag in bit 8 of byte 5
    int32_t temp           = b[5] >> 1;          // Temperature magnitude in degrees C in first 7 bits of byte 6
    int32_t temp_sign      = b[5] & 0x01;        // Temperature sign in bit 8 of byte 6
    int32_t volt1          = (b[7] << 8) | b[6]; // Current voltage
    int32_t volt2          = (b[9] << 8) | b[8]; // Previous starting voltage
                                             //
    if (temp_sign == 1) {
