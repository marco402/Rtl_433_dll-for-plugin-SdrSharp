...
/** @fn int32_t infactory_decode(r_device *decoder, bitbuffer_t *bitbuffer, int32_t startPulses, uint16_t package_type)
inFactory outdoor temperature and humidity sensor.
...
static int32_t infactory_crc_check(uint8_t *b)
{
...
static int32_t infactory_decode(r_device *decoder, bitbuffer_t *bitbuffer, int32_t startPulses, uint16_t package_type)
{
    ...
    int32_t id          = b[0];
    int32_t button      = (b[1] >> 3) & 1;
    int32_t battery_low = (b[1] >> 2) & 1;
    int32_t temp_raw    = (b[2] << 4) | (b[3] >> 4);
    int32_t humidity    = (b[3] & 0x0F) * 10 + (b[4] >> 4); // BCD, 'A0'=100%rH
    int32_t channel     = b[4] & 0x03;
