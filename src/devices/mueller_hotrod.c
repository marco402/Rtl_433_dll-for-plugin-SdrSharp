...
static int32_t mueller_hotrod_decode(r_device *decoder, bitbuffer_t *bitbuffer, int32_t startPulses, uint16_t package_type)
{
    ...
    // Find the preamble
    uint32_t pos = bitbuffer_search(bitbuffer, 0, 0, preamble_pattern, sizeof(preamble_pattern) * 8);
    ...
    int32_t crc_calc = crc8(b, 8, 0x07, 0x00) ^ 0x55;
    if (crc_calc != b[8]) {
    ...
    int32_t volume = ((b[4] & 0xf0) >> 4)*1000000+(b[4] & 0x0f)*100000+((b[5] & 0xf0) >> 4)*10000+(b[5] & 0x0f)*1000+((b[6] & 0xf0) >> 4)*100+(b[6] & 0x0f)*10+((b[7] & 0xf0) >> 4);
    int32_t flag   = b[7] & 0x0f;
