...
static int32_t oria_wa150km_decode(r_device *decoder, bitbuffer_t *bitbuffer, int32_t startPulses, uint16_t package_type)
{
    // Find a valid row (skipping short preamble rows)
    int32_t row;
    for (row = 0; row < bitbuffer->num_rows; row++) {
        if (bitbuffer->bits_per_row[row] == ORIA_WA150KM_BITLEN) {
            break;
        }
    }
    ...
    if (b[bitbuffer->bits_per_row[row]/8 - 1] != 0x69) {
    ...
    bitbuffer_manchester_decode(bitbuffer, row, 0, &manchester_buffer, ORIA_WA150KM_BITLEN);
    ...
    /* clang-format on */
	row = 0;
    decoder_output_data(decoder, data, &manchester_buffer, row, 0, startPulses, package_type);
