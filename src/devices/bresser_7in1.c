...
static int32_t bresser_7in1_decode(r_device *decoder, bitbuffer_t *bitbuffer, int32_t startPulses, uint16_t package_type)
{
    ...
    int32_t digest = lfsr_digest16(&msg[2], 23, 0x8810, 0xba95);
    if ((chk ^ digest) != 0x6df1) {
        decoder_logf(decoder, 2, __func__, "Digest check failed %04x vs %04x (%04x)", chk, digest, chk ^ digest);
        return DECODE_FAIL_MIC;
    }

    int32_t id          = (msg[2] << 8) | (msg[3]);
    int32_t flags       = (msg[15] & 0x0f);
    int32_t battery_low = (flags & 0x06) == 0x06;
	int32_t row = 0;
	if ((s_type == SENSOR_TYPE_WEATHER) || (s_type == SENSOR_TYPE_WEATHER3) || (s_type == SENSOR_TYPE_WEATHER8)) {
        int32_t wdir     = (msg[4] >> 4) * 100 + (msg[4] & 0x0f) * 10 + (msg[5] >> 4);
        int32_t wgst_raw = (msg[7] >> 4) * 100 + (msg[7] & 0x0f) * 10 + (msg[8] >> 4);
        int32_t wavg_raw = (msg[8] & 0x0f) * 100 + (msg[9] >> 4) * 10 + (msg[9] & 0x0f);
        int32_t rain_raw = (msg[10] >> 4) * 100000 + (msg[10] & 0x0f) * 10000 + (msg[11] >> 4) * 1000
                + (msg[11] & 0x0f) * 100 + (msg[12] >> 4) * 10 + (msg[12] & 0x0f) * 1; // 6 digits
        float rain_mm = rain_raw * 0.1f;
        int32_t temp_raw = (msg[14] >> 4) * 100 + (msg[14] & 0x0f) * 10 + (msg[15] >> 4);
        float temp_c = temp_raw * 0.1f;

        if (temp_raw > 600)
            temp_c = (temp_raw - 1000) * 0.1f;
        int32_t humidity = (msg[16] >> 4) * 10 + (msg[16] & 0x0f);
        int32_t lght_raw = (msg[17] >> 4) * 100000 + (msg[17] & 0x0f) * 10000 + (msg[18] >> 4) * 1000
                + (msg[18] & 0x0f) * 100 + (msg[19] >> 4) * 10 + (msg[19] & 0x0f);
        int32_t uv_raw =   (msg[20] >> 4) * 100 + (msg[20] & 0x0f) * 10 + (msg[21] >> 4);

        float light_klx = lght_raw * 0.001f; // TODO: remove this
        float light_lux = (float)lght_raw;
        float uv_index = uv_raw * 0.1f;

		// 3-in-1 (type 12) features Temp/Hum/Rain only
		int32_t wind_light_ok = s_type != SENSOR_TYPE_WEATHER3;
        ...
        int32_t pm_2_5      = (msg[10] & 0x0f) * 1000 + (msg[11] >> 4) * 100 + (msg[11] & 0x0f) * 10 + (msg[12] >> 4);
        int32_t pm_10       = (msg[12] & 0x0f) * 1000 + (msg[13] >> 4) * 100 + (msg[13] & 0x0f) * 10 + (msg[14] >> 4);
        int32_t pm_2_5_init = (msg[10] & 0x0f) == 0x0f; // confirmed by https://github.com/merbanan/rtl_433/issues/2816#issuecomment-1935439318
        int32_t pm_10_init  = (msg[12] & 0x0f) == 0x0f; // confirmed by https://github.com/merbanan/rtl_433/issues/2816#issuecomment-1935439318
        ...
        int32_t co2      = ((msg[4]& 0xf0) >> 4) * 1000 + (msg[4]& 0x0f) * 100 + ((msg[5]& 0xf0) >> 4) * 10 + (msg[5] & 0x0f);
        int32_t co2_init = (msg[5] & 0x0f) == 0x0f;
        ...
        int32_t hcho      = ((msg[4]& 0xf0) >> 4) * 1000 + (msg[4]& 0x0f) * 100 + ((msg[5]& 0xf0) >> 4) * 10 + (msg[5] & 0x0f);
        int32_t voc       = (msg[22]& 0x0f);
        int32_t hcho_init = (msg[5] & 0x0f) == 0x0f;
        int32_t voc_init  = voc == 0x0f;
