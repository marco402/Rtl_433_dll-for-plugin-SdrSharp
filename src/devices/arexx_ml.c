...
static int32_t arexx_ml_decode(r_device *decoder, bitbuffer_t *bitbuffer, int32_t startPulses, uint16_t package_type)
{
   ...
   if (bitbuffer->bits_per_row[1] != 0) {
    ...
    for (row = 0; row < bitbuffer->num_rows; ++row) {
        uint32_t pos = bitbuffer_search(bitbuffer, row, 0, preamble, 24);
        pos += 24;

        if (pos + 64 > bitbuffer->bits_per_row[row])
    ...
    int32_t chk = crc8le(b, msg_len, 0x31, 0x00);
    ...
    int32_t id       = (b[2] << 8) | (b[1]); // little-endian
    int32_t sens_val = (b[3] << 8) | (b[4]); // big-endian?
	...
	if (msg_len == 5 && (id & 0xF000) == 0x2000) {
		...
		// SHT10 Humidity
		humidity =  (float)(-2.0468f + 0.0367f * sens_val - 1.5955E-6 * sens_val * sens_val);
	}
	else if (msg_len == 6) {
		is_temp = is_alert = 1;
		// MCP9808 Ambient Temperature Register "5-4":
		temp_alert = (sens_val >> 13) & 7;
		int32_t temp_raw = (int16_t)(sens_val << 3); // uses sign-extend
		temp_c = (float)(temp_raw / 128.0f);
	}
...
static uint8_t const *const arexx_ml_output_fields[] = {
        "model",
        "id",
        "temperature_C",
        "temperature_alert",
