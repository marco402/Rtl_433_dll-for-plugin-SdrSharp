...
*/
static int32_t bresser_st1005h_decode(r_device *decoder, bitbuffer_t *bitbuffer, int32_t startPulses, uint16_t package_type)
{
	uint32_t nbRepeat = 3;
	if (decoder->_sourceIsFile)
		nbRepeat = 0;
    int32_t row = bitbuffer_find_repeated_row(bitbuffer, nbRepeat, 38);
    if (row < 0 || bitbuffer->bits_per_row[row] > 38) {
    ...
    uint8_t msg[4];
    bitbuffer_extract_bytes(bitbuffer, row, 1, msg, 4*8);
    msg[3] &= 0xfe; // remove last bit, it is part of the checksum
    int32_t chk = b[4] >> 2;
    int32_t sum = add_nibbles(msg, 4);
    ...
    int32_t id          = msg[0];
    int32_t battery_low = msg[1] >> 7;
    int32_t button      = (msg[1] >> 6) & 0x1;
    int32_t channel     = ((msg[1] >> 4) & 0x3) + 1;
    int32_t temp_raw    = (int16_t)(((msg[1] & 0xf) << 12) | (msg[2] << 4)); // uses sign-extend from 12 bit
    double temp_c   = (temp_raw >> 4) * 0.1;
    int32_t humidity    = msg[3] >> 1;
