...
/** @fn int32_t watts_thermostat_decode(r_device *decoder, bitbuffer_t *bitbuffer, int32_t startPulses, uint16_t package_type)
Watts WFHT-RF Thermostat.
...
static int32_t watts_thermostat_decode(r_device *decoder, bitbuffer_t *bitbuffer , int32_t startPulses, uint16_t package_type)
{
    ...
    for (uint16_t row = 0; row < bitbuffer->num_rows; ++row) {
        ...
        uint32_t bitpos = bitbuffer_search(bitbuffer, row, 0, preamble_pattern, WATTSTHERMO_PREAMBLE_BITLEN);
        if (bitpos >= row_len) {
        ...
        int32_t id  = (id_raw[1] << 8) | id_raw[0];
        ...
        int32_t pairing = flags[0] & WF_PAIRING;
        ...
        int32_t temp = (temp_raw[1] << 8) | temp_raw[0];
        ...
        int32_t setp = (setp_raw[1] << 8) | setp_raw[0];
