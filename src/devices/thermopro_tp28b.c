...
/** @fn int32_t thermopro_tp28b_decode(r_device *decoder, bitbuffer_t *bitbuffer, int32_t startPulses, uint16_t package_type)
ThermoPro TP28b Super Long Range Wireless Meat Thermometer for Smoker BBQ Grill.
...
static int32_t thermopro_tp28b_decode(r_device *decoder, bitbuffer_t *bitbuffer, int32_t startPulses, uint16_t package_type)
{
    ...
    int32_t offset = bitbuffer_search(bitbuffer, 0, 0,
    ...
    int32_t sum = (add_bytes(b, 16) & 0xff) - b[16];
    if (sum) {
