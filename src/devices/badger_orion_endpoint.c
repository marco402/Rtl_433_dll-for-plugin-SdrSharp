...
*/
static int32_t orion_endpoint_decode(r_device *decoder, bitbuffer_t *bitbuffer, int32_t startPulses, uint16_t package_type)
{
    ...
    int32_t offset = bitbuffer_search(bitbuffer, 0, 0, preamble_pattern, PREAMBLE_BITLEN);
    ...
    for (int32_t i = 0; i < DATA_BYTELEN; i++) {
    ...
    uint8_t const *endpoint_model = "Unknown Model";
