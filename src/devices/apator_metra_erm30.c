...
static int32_t apator_metra_erm30_decode(r_device *decoder, bitbuffer_t *bitbuffer, int32_t startPulses, uint16_t package_type)
{
	...
	int32_t row = 0;
	uint32_t start_pos = bitbuffer_search(bitbuffer, row, 0, preamble, 8 * sizeof(preamble));
...
static void decrypt_payload(uint8_t plen, uint8_t *payload_encr, uint8_t *payload_decr, uint8_t *decr_mask)
{
    ...
    for (int32_t i = 0; i < 2 * plen; i++) {
        uint8_t nibble_encr, nibble_decr, nibble_mask;

        uint32_t bitshift = (i % 2) ? 0 : 4;
