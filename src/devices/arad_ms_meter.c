...
static int32_t arad_mm_dialog3g_decode(r_device *decoder, bitbuffer_t *bitbuffer, int32_t startPulses, uint16_t package_type)
{
    ...
    uint32_t start_pos = bitbuffer_search(bitbuffer, row, 0, preamble_pattern, 48);
    ...
    int32_t serno    = b[0] | (b[1] << 8) | (b[2] << 16); // 24 bit little endian Meter Serial number
    int32_t wreadraw = b[5] | (b[6] << 8) | (b[7] << 16); // 24 bit little endian Meter water consumption reading
