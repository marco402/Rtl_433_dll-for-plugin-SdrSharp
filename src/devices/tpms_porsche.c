...
static int32_t tpms_porsche_decode(r_device *decoder, bitbuffer_t *bitbuffer, int32_t row, uint32_t bitpos, int32_t startPulses, uint16_t package_type)
{
...
static int32_t tpms_porsche_callback(r_device *decoder, bitbuffer_t *bitbuffer, int32_t startPulses, uint16_t package_type)
{
    ...
    uint32_t bitpos = 0;
    while ((bitpos = bitbuffer_search(bitbuffer, row, bitpos, preamble_pattern, 20)) + 100 <=
            bitbuffer->bits_per_row[row]) {
        events += tpms_porsche_decode(decoder, bitbuffer, row, bitpos + 20, startPulses, package_type);
		bitpos += 2;
    }
