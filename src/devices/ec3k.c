...
static int32_t ec3k_extract_fields(r_device *decoder, bitbuffer_t *packetbuffer, int32_t startPulses, uint16_t package_type);
 ...
 */
static int32_t ec3k_decode(r_device *decoder, bitbuffer_t *bitbuffer, int32_t startPulses, uint16_t package_type)
{
    ...
    for (int32_t bufferpos = 17; rc != 1 && bufferpos < bitbuffer->bits_per_row[0]; bufferpos++) {
        ...
        if (ctx.packetpos >= DECODED_PAKET_LEN_BYTES) {
            rc = ec3k_extract_fields(decoder, (bitbuffer_t *)packetbuffer, startPulses, package_type);
            ec3k_resync_decoder(&ctx);
        }
    }
 ...
 static int32_t ec3k_extract_fields(r_device *decoder, bitbuffer_t *packetbuffer, int32_t startPulses, uint16_t package_type)
{
    uint16_t id             = unpack_nibbles((const uint8_t *)packetbuffer, 1, 4);
    uint16_t time_total_low = unpack_nibbles((const uint8_t *)packetbuffer, 5, 4);
    uint16_t pad_1          = unpack_nibbles((const uint8_t *)packetbuffer, 9, 4);
    uint16_t time_on_low    = unpack_nibbles((const uint8_t *)packetbuffer, 13, 4);
    uint32_t pad_2          = unpack_nibbles((const uint8_t *)packetbuffer, 17, 7);
    uint32_t energy_low     = unpack_nibbles((const uint8_t *)packetbuffer, 24, 7);
    double power_current    = unpack_nibbles((const uint8_t *)packetbuffer, 31, 4) / 10.0;
    double power_max        = unpack_nibbles((const uint8_t *)packetbuffer, 35, 4) / 10.0;
    // unknown? (seems to be used for internal calculations)
    uint32_t energy2 = unpack_nibbles((const uint8_t *)packetbuffer, 39, 6);
    // 						nibbles[45:59]
    uint16_t time_total_high = unpack_nibbles((const uint8_t *)packetbuffer, 59, 3);
    uint32_t pad_3           = unpack_nibbles((const uint8_t *)packetbuffer, 62, 5);
    uint64_t energy_high     = (uint64_t)unpack_nibbles((const uint8_t *)packetbuffer, 67, 4) << 28;
    uint16_t time_on_high    = unpack_nibbles((const uint8_t *)packetbuffer, 71, 3);
    uint8_t reset_counter    = unpack_nibbles((const uint8_t *)packetbuffer, 74, 2);
    uint8_t flags            = unpack_nibbles((const uint8_t *)packetbuffer, 76, 1);
    uint8_t pad_4            = unpack_nibbles((const uint8_t *)packetbuffer, 77, 1);
    uint16_t received_crc    = 0xffff ^ (unpack_nibbles((const uint8_t *)packetbuffer, 78, 2) | (unpack_nibbles((const uint8_t *)packetbuffer, 80, 2) << 8)); // little-endian
    uint16_t calculated_crc  = crc16lsb((const uint8_t *)packetbuffer, DECODED_PAKET_LEN_BYTES - 2, 0x8408, 0xffff);
    ...
    decoder_output_data(decoder, data, packetbuffer, 0, 0, startPulses, package_type);
    return 1;
}
