...
static int32_t vevor_7in1_decode(r_device *decoder, bitbuffer_t *bitbuffer, int32_t startPulses, uint16_t package_type)
{
    ...
    while ((pos = bitbuffer_search(bitbuffer, 0, pos, preamble_pattern, sizeof(preamble_pattern) * 8)) + VEVOR_MESSAGE_BITLEN <= bitbuffer->bits_per_row[0]) {
        ...
        int32_t battery_low = (b[4] & 0x80) >> 7;

        if (b[0] == 0xAA && b[1] == 0) {

            int32_t temp_raw      = (b[5] << 8) | b[6];
            float temp_c      = (temp_raw - 500) * 0.1f;
            int32_t humidity      = b[7];
            int32_t wind_raw      = ((b[8] << 8) | b[9]) - 257; // need to remove 0x0101.
            float speed_kmh   = wind_raw / 10.0f  ; // wind_raw / 36.0f for m/s
            int32_t gust_raw      = b[10];
            float gust_kmh    = gust_raw / 1.5f ; // gust_raw / 1.5f / 3.6f m/s, + 0.1f offset from the weather display
            int32_t direction_deg = (((b[11] & 0x0f) << 8) | b[12]) - 257; // need to remove 0x101.
            int32_t rain_raw      = ((b[13] << 8) | b[14]) - 257; // need to remove 0x101.
            float rain_mm     = rain_raw * 0.233f; // calculation is 0.43f but display is 0.5f
            int32_t uv_index      = (b[15] & 0x1f) - 1;
            int32_t light_lux     = ((b[16] << 8) | b[17]) - 257; // need to remove 0x0101.
            int32_t lux_multi     = (light_lux & 0x8000) >> 15;
