...
static int32_t decode_xc0324_message(r_device *decoder, bitbuffer_t *bitbuffer,
	uint32_t row, uint16_t bitpos, data_t **data_out)
{
    // Extract the message
...
*/
static int32_t digitech_xc0324_decode(r_device *decoder, bitbuffer_t *bitbuffer, int32_t startPulses, uint16_t package_type)
{
    ...
    for (row = 0; row < bitbuffer->num_rows; ++row) {
        if (bitbuffer->bits_per_row[row] < XC0324_MESSAGE_BITLEN) {
            ...
            decoder_logf_bitrow(decoder, 1, __func__, bitbuffer->bb[row], bitbuffer->bits_per_row[row],
                    ...
                    XC0324_MESSAGE_BITLEN, bitbuffer->bits_per_row[row], row, 0);
            continue; // DECODE_ABORT_LENGTH
        }
        // We have enough bits so search for a message preamble followed by
        // enough bits that it could be a complete message.
		uint32_t bitpos = 0;
        while ((bitpos = bitbuffer_search(bitbuffer, row, bitpos, preamble_pattern, 8)) + XC0324_MESSAGE_BITLEN <=
                bitbuffer->bits_per_row[row]) {
            ret = decode_xc0324_message(decoder, bitbuffer, row, bitpos, &data);
            if (ret > 0) {
                events += ret;
            }
