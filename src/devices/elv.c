...
based on fs20.c
*/
static int32_t em1000_callback(r_device *decoder, bitbuffer_t *bitbuffer, int32_t startPulses, uint16_t package_type)
{
    bitrow_t *bb = bitbuffer->bb;
    uint8_t dec[10];
    uint8_t bit = 18; // preamble
    uint8_t bb_p[14];
    //uint8_t* types[] = {"EM 1000-S", "EM 1000-?", "EM 1000-GZ"};
    ...
    for (i = 0; i < 9; i++) {
        dec[i] = (uint8_t)AD_POP(bb_p, 8, bit);
        bit += 8;
        uint8_t stopbit = (uint8_t)AD_POP(bb_p, 1, bit);
        bit ++;
        if (!stopbit) {
//            decoder_logf(decoder, 0, __func__, "!stopbit: %d", i);
    ...
    // Read checksum
    checksum_received = (uint8_t)AD_POP(bb_p, 8, bit); // bit+=8;
...
//    decoder_log_bitrow(decoder, 0, __func__, dec, 9 * 8, "");
    ...
    //uint8_t *subtype = dec[0] >= 1 && dec[0] <= 3 ? types[dec[0] - 1] : "?";
    int32_t code      = dec[1];
    int32_t seqno     = dec[2];
    int32_t total     = dec[3] | dec[4] << 8;
    int32_t current   = dec[5] | dec[6] << 8;
    int32_t peak      = dec[7] | dec[8] << 8;
...
static uint8_t const *const elv_em1000_output_fields[] = {
        "model",
        "id",
        "seq",
        "total",
 ...
 */
static int32_t ws2000_callback(r_device *decoder, bitbuffer_t *bitbuffer, int32_t startPulses, uint16_t package_type)
{
    bitrow_t *bb = bitbuffer->bb;
    uint8_t dec[16]= {0};
    uint8_t nibbles=0;
    uint8_t bit=11; // preamble
    uint8_t const *const types[]={"!AS3", "AS2000/ASH2000/S2000/S2001A/S2001IA/ASH2200/S300IA", "!S2000R", "!S2000W", "S2001I/S2001ID", "!S2500H", "!Pyrano", "KS200/KS300"};
    ...
    dec[0] = (uint8_t)AD_POP(bb[0], 4, bit); bit+=4;
    stopbit= (uint8_t)AD_POP(bb[0], 1, bit); bit+=1;
    if (!stopbit) {
    ...
    for (i = 1; i <= length[dec[0]]; i++) {
        dec[i] = (uint8_t)AD_POP(bb[0], 4, bit); bit+=4;
        stopbit= (uint8_t)AD_POP(bb[0], 1, bit); bit+=1;
        if (!stopbit) {
    ...
    // Read sum
    sum_received = (uint8_t) AD_POP(bb[0], 4, bit); // bit+=4;
    sum_calculated+=5;
    sum_calculated&=0xF;
    ...
    uint8_t const *subtype  = (dec[0] <= 7) ? types[dec[0]] : "?";
    int32_t code       = dec[1] & 7;
...
static uint8_t const *const elv_ws2000_output_fields[] = {
        "model",
        "id",
        "subtype",
        "temperature_C",
