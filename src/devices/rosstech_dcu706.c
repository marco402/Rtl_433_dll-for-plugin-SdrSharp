...
/** @fn int32_t rosstech_dcu706_decode(r_device *decoder, bitbuffer_t *bitbuffer, int32_t startPulses, uint16_t package_type)
...
static int32_t rosstech_dcu706_decode(r_device *decoder, bitbuffer_t *bitbuffer, int32_t startPulses, uint16_t package_type)
{
    ...
    int32_t const preamble_length = 11;
	...
	uint32_t start_pos = bitbuffer_search(bitbuffer, 0, 0, preamble_data_transmission, preamble_length);
    ...
    uint8_t b[5] = {0};
	uint32_t r   = extract_bytes_uart_parity(msg, 0, 55, b);
    if (r != 5) {
        decoder_logf(decoder, 2, __func__, "UART decoding failed. Got %d of 5 bytes", r);
        return DECODE_ABORT_LENGTH;
    }

    int32_t msg_type = b[0];                 // S
    int32_t id       = (b[1] << 8) | (b[2]); // I
    int32_t temp_f   = b[3];                 // T
    int32_t checksum = b[4];                 // C
