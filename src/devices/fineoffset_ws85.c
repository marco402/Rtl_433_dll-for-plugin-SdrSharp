...
static int32_t fineoffset_ws85_decode(r_device *decoder, bitbuffer_t *bitbuffer, int32_t startPulses, uint16_t package_type)
{
    ...
    int32_t battery_mv  = (b[4] * 20); // mV
    int32_t flags       = b[5]; // to find the wind msb
    int32_t wind_avg    = ((b[5] & 0x10) << 4) | (b[7]);
    int32_t wind_dir    = ((b[5] & 0x20) << 3) | (b[8]);
    int32_t wind_max    = ((b[5] & 0x40) << 2) | (b[9]);
    int32_t rain_start  = (b[12] & 0x10) >> 4;
    int32_t rain_raw    = (b[15] << 8) | b[16]; // Extract the 16-bit raw value
    int32_t supercap_V  = (b[17] & 0x3f);
    int32_t firmware    = b[25];

    int32_t battery_ok = 0;
    if (battery_mv > 2400) {
        battery_ok = 1;
    }
