...
static void print_row_bytes(uint8_t *row_bytes, uint8_t *bits, int32_t num_bits)
{
    row_bytes[0] = '\0';
    // print byte-wide
    for (int32_t col = 0; col < (num_bits + 7) / 8; ++col) {
...
*/
static int32_t flex_callback(r_device *decoder, bitbuffer_t *bitbuffer, int32_t startPulses, uint16_t package_type)
{
    int32_t i;
    ...
    if (params->preamble_len) {
        ...
        match_count = 0;
        for (i = 0; i < bitbuffer->num_rows; i++) {
            bit_offset = bitbuffer_search(bitbuffer, i, 0, params->preamble_bits, params->preamble_len);
            if (bit_offset < bitbuffer->bits_per_row[i]) {
                if (row < 0)
                    row = i;
                match_count++;
                bit_offset += params->preamble_len;
                // TODO: refactor to bitbuffer_shift_row()
                uint32_t len = bitbuffer->bits_per_row[i] - bit_offset;
                bitbuffer_t tmp = {0};
                bitbuffer_extract_bytes(bitbuffer, row, bit_offset, tmp.bb[0], len);
    ...
    // discard duplicates
    if (params->unique) {
        print_row_bytes(row_bytes, bitbuffer->bb[row], bitbuffer->bits_per_row[row]);
        ...
        data = data_make(
                ...
                "len", "", DATA_INT, bitbuffer->bits_per_row[row],
...
static float parse_atoiv(uint8_t const *str, int32_t def, uint8_t const *error_hint)
{
	if (!str) {
		return (float)def;
	}
...
static float parse_float(uint8_t const *str, uint8_t const *error_hint)
{
	if (!str) {
...
static uint32_t parse_modulation(uint8_t const *str)
{
...
static uint32_t parse_bits(const uint8_t *code, uint8_t *bitrow)
{
    bitbuffer_t bits = {0};
    ...
    uint32_t len = bits.bits_per_row[0];
    if (len > 1024) {
...
static uint32_t parse_symbol(const uint8_t *code)
{
    bitbuffer_t bits = {0};
    bitbuffer_parse(&bits, code);
    if (bits.num_rows != 1) {
...
static const uint8_t *parse_map(const uint8_t *arg, struct flex_get *getter)
{
    const uint8_t *c = arg;
    int32_t i = 0;
    while (*c == ' ') c++;
    ...
    while (*c) {
        ...
        key = strtol(c, (uint8_t **)&c, 0); // hex, oct, or dec
...
static void parse_getter(const uint8_t *arg, struct flex_get *getter)
{
    uint8_t bitrow[128];
    while (arg && *arg) {
        if (*arg == '[') {
        ...
        if (*arg == '@')
            ...
            getter->format = _strdup(arg);
            if (!getter->format)
                FATAL_STRDUP("parse_getter()");
        }
        else {
            getter->name = _strdup(arg);
            if (!getter->name)
                FATAL_STRDUP("parse_getter()");
        }
        arg = p;
...
r_device *flex_create_device(uint8_t *spec);
...
r_device *flex_create_device(uint8_t *spec)
{
    ...
    while (getkwargs(&spec, &key, &val)) {
        ...
        if (!key || !*key)
            ...
            params->name = _strdup(val);
            if (!params->name)
                FATAL_STRDUP("flex_create_device()");
            int32_t name_size = (int32_t)strlen(val) + 27;
            uint8_t* flex_name = malloc(name_size);
            if (!flex_name)
			...
			dev->priority = (uint32_t) parse_atoiv(val, 0, "priority: ");

		else if (!strcasecmp(key, "bits>"))
			params->min_bits = (uint32_t)parse_atoiv(val, 0, "bits: ");
		else if (!strcasecmp(key, "bits<"))
			params->max_bits = (uint32_t)parse_atoiv(val, 0, "bits: ");
		else if (!strcasecmp(key, "bits"))
			params->min_bits = params->max_bits = (uint32_t)parse_atoiv(val, 0, "bits:");

		else if (!strcasecmp(key, "rows>"))
			params->min_rows = (uint32_t)parse_atoiv(val, 0, "rows: ");
		else if (!strcasecmp(key, "rows<"))
			params->max_rows = (uint32_t)parse_atoiv(val, 0, "rows: ");
		else if (!strcasecmp(key, "rows"))
			params->min_rows = params->max_rows = (uint32_t)parse_atoiv(val, 0, "rows: ");

		else if (!strcasecmp(key, "repeats>"))
			params->min_repeats = (uint32_t)parse_atoiv(val, 0, "repeats: ");
		else if (!strcasecmp(key, "repeats<"))
			params->max_repeats = (uint32_t)parse_atoiv(val, 0, "repeats: ");
		else if (!strcasecmp(key, "repeats"))
			params->min_repeats = params->max_repeats = (uint32_t)parse_atoiv(val, 0, "repeats: ");

		else if (!strcasecmp(key, "invert"))
			params->invert = (uint32_t)parse_atoiv(val, 1, "invert: ");
		else if (!strcasecmp(key, "reflect"))
			params->reflect = (uint32_t)parse_atoiv(val, 1, "reflect: ");
			...
			params->count_only = (uint32_t)parse_atoiv(val, 1, "countonly: ");

		else if (!strcasecmp(key, "unique"))
			params->unique = (uint32_t)parse_atoiv(val, 1, "unique: ");
    ...
    if (params->unique) {
        int32_t i = 0;
        for (int32_t f = 0; output_fields[f]; ++f) {
            params->fields[i++] = output_fields[f];
        }
        params->fields[i++] = "len";
        params->fields[i++] = "data";
        for (int32_t g = 0; g < GETTER_SLOTS && params->getter[g].name; ++g) {
