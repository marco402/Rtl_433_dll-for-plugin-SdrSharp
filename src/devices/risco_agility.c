...
/** @fn int32_t risco_agility_decode(r_device *decoder, bitbuffer_t *bitbuffer, int32_t startPulses, uint16_t package_type)
Risco 2 way Agility protocol.
...
static int32_t gray_decode(int32_t n) {
    int32_t p = n;
    while (n >>= 1) p ^= n;
    return p;
}

static int32_t risco_agility_decode(r_device *decoder, bitbuffer_t *bitbuffer, int32_t startPulses, uint16_t package_type)
{
    ...
    int32_t pos = bitbuffer_search(bitbuffer, 0, 0, preamble_pattern, sizeof(preamble_pattern) * 8);
    ...
    int32_t message_type = (b[0] << 8)| b[1];
    ...
    int32_t id = (b[6] << 16) | (b [7] << 8) | b[8];

    reflect_bytes(b,16);

    // Alarm state, 0x4 = Tampered, 0x6 = Tampered_motion, 0x2 = Motion, 0x0 = Clear, not detection.
    int32_t state        = gray_decode(b[12] & 0xF);
    int32_t tamper       = (state & 0x4) >> 2;
    int32_t motion       = (state & 0x2) >> 1;
    int32_t low_batt     = (gray_decode((b[12] & 0xF0) >> 4) & 0x8)>> 3;
    int32_t counter_raw  = (b[5] << 8) | b[4];
    int32_t counter   = gray_decode(counter_raw);
