...
*/
static int32_t thermopro_tx7b_decode(r_device *decoder, bitbuffer_t *bitbuffer, int32_t startPulses, uint16_t package_type)
{
	...
	int32_t checksum = lfsr_digest8_reverse(b, 8, 0x98, 0x25);
	...
	int32_t low_bat   = b[1] >> 7;
	int32_t tx_button = (b[1] & 0x40) >> 6;
...
static uint8_t const *const tx7b_output_fields[] = {
        "model",
        "id",
        "battery_ok",
        "button",
