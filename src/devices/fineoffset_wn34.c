...
static int32_t fineoffset_wn34_decode(r_device *decoder, bitbuffer_t *bitbuffer, int32_t startPulses, uint16_t package_type)
{
    data_t *data;
    ...
    int32_t temp_raw    = (int16_t)((b[4] & 0x0F) << 12 | b[5] << 4); // use sign extend
    int32_t sub_type    = (b[4] & 0xF0) >> 4;
    ...
    int32_t battery_mv  = (b[6] & 0x7f) * 20;         // mV
