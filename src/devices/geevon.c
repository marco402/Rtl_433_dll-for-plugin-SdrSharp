...
static int32_t geevon_callback(r_device *decoder, bitbuffer_t *bitbuffer, int32_t startPulses, uint16_t package_type)
{
    // invert all the bits
    ...
    if (bitbuffer->bits_per_row[row] != 73) {
        return DECODE_ABORT_LENGTH;
    }
    ...
    int32_t battery_low = (b[1] >> 7);              // 0x00: battery good, 0x80: battery low
    int32_t channel     = ((b[1] & 0x30) >> 4) + 1; // channel: 1, 2, 3
