...
*/
static int32_t fineoffset_wh43_decode(r_device *decoder, bitbuffer_t *bitbuffer, int32_t startPulses, uint16_t package_type)
{
    uint8_t const preamble[] = {0xAA, 0x2D, 0xD4};

	uint32_t bit_offset = bitbuffer_search(bitbuffer, 0, 0, preamble, sizeof(preamble) * 8) + sizeof(preamble) * 8;
    uint8_t b[10];
    ...
    int32_t crc = crc8(b, 8, 0x31, 0x00);  // Compute CRC over first 8 bytes.
    int32_t sum = add_bytes(b, 9) & 0xff;
    if (crc != b[8] || sum != b[9]) {
    ...
    // int32_t family     = b[0];
    int32_t id         = (b[1] << 16) | (b[2] << 8) | b[3]; // 24-bit ID
    int32_t pm25       = ((b[4] & 0x3F) << 8) | b[5];
    int32_t pm100      = ((b[6] & 0x3F) << 8) | b[7];
    int32_t batt_bars  = ((b[4] & 0x40) >> 4) | ((b[6] & 0xC0) >> 6);
    int32_t ext_power  = batt_bars == 6;
