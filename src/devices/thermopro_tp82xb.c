...
*/
static int32_t thermopro_tp828b_decode(r_device *decoder, bitbuffer_t *bitbuffer, int32_t startPulses, uint16_t package_type)
{
    ...
    int32_t msg_len = bitbuffer->bits_per_row[0];
    ...
    int32_t offset = bitbuffer_search(bitbuffer, 0, 0, preamble_pattern, sizeof(preamble_pattern) * 8);
    ...
    for (int32_t p = 10; p != -1; p += -1)
        b_reflect[10 - p] = b[p];
    int32_t checksum = lfsr_digest8(b_reflect, 11, 0x98, 0x16) ^ 0xac ;
    if (checksum != b[11]) {
    ...
    int32_t id        = b[0];
    int32_t display_u = (b[1] & 0xF0) >> 4;
    int32_t flags     = b[1] & 0xF;
    int32_t p1_raw    = b[2] << 4 | (b[3] & 0xF0) >> 4;
    int32_t p1_lo_raw = (b[3] & 0x0F) << 8 | b[4];
    int32_t p1_hi_raw = b[5] << 4 | (b[6] & 0xF0) >> 4;
    int32_t p2_raw    = (b[6] & 0x0F) << 8 | b[7];
    int32_t p2_lo_raw = b[8] << 4 | (b[9] & 0xF0) >> 4;
    int32_t p2_hi_raw = (b[9] & 0x0F) << 8 | b[10];
...
*/
static int32_t thermopro_tp829b_decode(r_device *decoder, bitbuffer_t *bitbuffer, int32_t startPulses, uint16_t package_type)
{
    uint8_t const preamble_pattern[] = { // 0xd2, removed to increase success
                                        0x55, 0x2d, 0xd4};
    ...
    uint8_t b_reflect[8];
    for (int32_t p = 7; p != -1; p += -1)
        b_reflect[7 - p] = b[p];
    int32_t checksum = lfsr_digest8(b_reflect, 8, 0x98, 0x55);
    if (checksum != b[8]) {
    ...
    int32_t flags     = b[1] & 0xF;
    int32_t p1_raw    = b[2] << 4 | (b[3] & 0xF0) >> 4;
    int32_t p2_raw    = (b[3] & 0x0F) << 8 | b[4];
    int32_t p3_raw    = b[5] << 4 | (b[6] & 0xF0) >> 4;
    int32_t p4_raw    = (b[6] & 0x0F) << 8 | b[7];
    float p1_temp = (p1_raw - 500) * 0.1f;
    float p2_temp = (p2_raw - 500) * 0.1f;
