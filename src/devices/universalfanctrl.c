...
static int32_t universalfan_decode(r_device *decoder, bitbuffer_t *bitbuffer, int32_t startPulses, uint16_t package_type)
{
    int32_t row = bitbuffer_find_repeated_row(bitbuffer, 3, 33);
    if (row < 0) {
        return DECODE_ABORT_LENGTH;
    }

    uint8_t *b  = bitbuffer->bb[row];
    // int32_t chk_msg = (b[3] & 0x0F);
    int32_t sum = xor_bytes(b, 4); // xor message bytes, last byte also has the checksum
    ...
    int32_t address = (b[0] << 12) + (b[1] << 4) + (b[2] >> 4);    // @0 {20};
    int32_t button  = ((b[2] & 0x0F) << 1) + ((b[3] & 0x80) >> 7); // @20 {5}
    int32_t counter = (b[3] & 0x7F) >> 4;                          // @25 {3}
    // int32_t rc      = (b[3] & 0x0F);                               // @28 {4}
    uint8_t const *button_str;
