...
*/
static int32_t maverick_xr50_decode(r_device *decoder, bitbuffer_t *bitbuffer, int32_t startPulses, uint16_t package_type)
{
    ...
    int32_t start_pos = bitbuffer_search(bitbuffer,0, 0, preamble, sizeof(preamble) * 8);
    ...
    int32_t p_1_flags    = (b[2] & 0xf0) >> 4;
    int32_t p_1_temp_raw = ((b[2] & 0x0f) << 8) | b[3];
    int32_t p_1_high_raw = (b[4] << 4) | ((b[5] & 0xf0) >> 4);
    int32_t p_1_low_raw  = ((b[5] & 0x0f) << 8) | b[6];

    int32_t p_2_flags    = (b[7] & 0xf0) >> 4;
    int32_t p_2_temp_raw = ((b[7] & 0x0f) << 8) | b[8];
    int32_t p_2_high_raw = (b[9] << 4) | ((b[10] & 0xf0) >> 4);
    int32_t p_2_low_raw  = ((b[10] & 0x0f) << 8) | b[11];

    int32_t p_3_flags    = (b[12] & 0xf0) >> 4;
    int32_t p_3_temp_raw = ((b[12] & 0x0f) << 8) | b[13];
    int32_t p_3_high_raw = (b[14] << 4) | ((b[15] & 0xf0) >> 4);
    int32_t p_3_low_raw  = ((b[15] & 0x0f) << 8) | b[16];

    int32_t p_4_flags    = (b[17] & 0xf0) >> 4;
    int32_t p_4_temp_raw = ((b[17] & 0x0f) << 8) | b[18];
    int32_t p_4_high_raw = (b[19] << 4) | ((b[20] & 0xf0) >> 4);
    int32_t p_4_low_raw  = ((b[20] & 0x0f) << 8) | b[21];
