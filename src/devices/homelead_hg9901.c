...
*/
static int32_t homelead_hg9901_decoder(r_device *decoder, bitbuffer_t *bitbuffer, int32_t startPulses, uint16_t package_type)
{
    uint8_t const preamble[] = {0x55, 0xaa};
    // Rough estimate of Lux values for light levels 0 - 8.
	int32_t const lux_estimate[] = {60, 200, 400, 600, 1000, 1500, 2800, 4500, 10000, -1, -1, -1, -1, -1, -1, -1};

	int32_t row = bitbuffer_find_repeated_row(bitbuffer, 1, 65); // expected are 12 repeats but 1 is enough
    if (row < 0) {
        return DECODE_ABORT_EARLY; // no good row found
    }

    // Check that bits_per_row is 65 or a few bits more
	uint32_t row_len = bitbuffer->bits_per_row[row];
    if (row_len > 65 + 8) {
        return DECODE_ABORT_EARLY; // wrong Data Length (must be 65)
    }

    // Search preamble
	uint32_t pos = bitbuffer_search(bitbuffer, row, 0, preamble, 16);
    if (pos + 65 > row_len) {
	...
	int32_t chk = (b[7] & 0xf0) >> 4;
	int32_t sum = add_nibbles(b, 7) & 0x0f;
	...
	int32_t moisture    = b[4];
	int32_t t_sign      = (b[5] & 0x80) >> 7;
	int32_t temperature = b[5] & 0x7f;
	int32_t sequence    = (b[6] & 0xc0) >> 6;
	int32_t batt_lvl    = (b[6] & 0x30) >> 4;
	int32_t light_lvl   = (b[6] & 0x0f);
	int32_t light_lux   = lux_estimate[light_lvl];
