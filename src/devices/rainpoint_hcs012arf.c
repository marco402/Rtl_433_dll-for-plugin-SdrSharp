...
static int32_t rainpoint_hcs012arf_decode(r_device *decoder, bitbuffer_t *bitbuffer, int32_t startPulses, uint16_t package_type)
{

    // Find repeats
	int32_t row = bitbuffer_find_repeated_row(bitbuffer, 4, 163);
    if (row < 0)
    ...
    // Checksum
	int32_t sum = add_bytes(&b[1], 8); // header not part of the sum
    if ((sum & 0xff) != b[9]) {
        decoder_logf(decoder, 2, __func__, "Checksum failed %04x vs %04x", b[9], sum);
        return DECODE_FAIL_MIC;
    }

	int32_t id       = (b[4] << 24) | (b[3] << 16) | (b[2] << 8) | b[1]; // just a guess, little-endian
	int32_t flags1   = b[5]; // may contains 1 bit MSB for Rain Gauge
	int32_t bat_low  = (flags1 & 0x02) >> 1;
    //int32_t bat_in   = (flags1 & 0x01); // power up, battery inserted = 1, then always 0
	int32_t flags2   = b[6];        // may contains 1 bit MSB for Rain Gauge
	int32_t rain_raw  = (b[8] << 8) | b[7]; // little-endian
