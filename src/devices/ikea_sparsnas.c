    ...
    for (k0=0;k0<0xFF;++k0) {
        ...
        for (k1=0;k1<0xFF;++k1) {
            ...
            for (k2=0;k2<0xFF;++k2) {
                ...
                dec_sensor_id = (uint32_t)d0 << 24 | d1 << 16 | d2 << 8 | d3;
                ...
                for (k4=0;k4<0xFF;++k4) {
                    key_sensor_id  = ((uint32_t)k0 << 24 | k4 << 16 | k2 << 8 | k1) + IKEA_SPARSNAS_ID_KEY_SUB;
...
static int32_t ikea_sparsnas_decode(r_device *decoder, bitbuffer_t *bitbuffer, int32_t startPulses, uint16_t package_type)
{
    ...
    uint32_t rcv_sensor_id = (uint32_t)decrypted[5] << 24 | decrypted[6] << 16 | decrypted[7] << 8 | decrypted[8];
    ...
    uint32_t pulses = ((uint32_t)decrypted[13] << 24 | decrypted[14] << 16 | decrypted[15] << 8 | decrypted[16]);
