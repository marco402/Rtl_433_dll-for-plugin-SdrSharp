...
/** @fn int32_t honeywell_cm921_decode(r_device *decoder, bitbuffer_t *bitbuffer, int32_t startPulses, uint16_t package_type)
...
static int32_t decode_10to8(uint8_t const *b, int32_t pos, int32_t end, uint8_t *out)
{
    // we need 10 bits
    if (pos + 10 > end) {
...
static void decode_device_id(const uint8_t device_id[3], uint8_t *buf, size_t buf_sz)
{
    int32_t dev_type = device_id[0] >> 2;
    int32_t dev_id   = (device_id[0] & 0x03) << 16 | (device_id[1] << 8) | device_id[2];

    uint8_t const *dev_name = " --";
...
static uint8_t next(const uint8_t *bb, uint32_t *ipos, uint32_t num_bytes)
{
...
static int32_t parse_msg(bitbuffer_t *bmsg, int32_t row, message_t *msg)
{
    if (!bmsg || row >= bmsg->num_rows || bmsg->bits_per_row[row] < 8) {
        return DECODE_ABORT_LENGTH;
    }

    uint32_t num_bytes = bmsg->bits_per_row[0]/8;
    uint32_t num_bits = bmsg->bits_per_row[0];
    uint32_t ipos = 0;
    const uint8_t *bb = bmsg->bb[row];
    memset(msg, 0, sizeof(message_t));

    // Checksum: All bytes add up to 0.
    int32_t bsum = add_bytes(bb, num_bytes) & 0xff;
    int32_t checksum_ok = bsum == 0;
    ...
    for (uint32_t i = 0; i < msg->num_device_ids; i++) {
    ...
    for (uint32_t i = 0; i < msg->payload_length; i++) {
        msg->payload[i] = next(bb, &ipos, num_bytes);
    }

    if (ipos < num_bits - 8) {
        uint32_t num_unparsed_bits = (bmsg->bits_per_row[row] - 8) - ipos;
...
static int32_t honeywell_cm921_decode(r_device *decoder, bitbuffer_t *bitbuffer, int32_t startPulses, uint16_t package_type)
{
    ...
    int32_t preamble_start = bitbuffer_search(bitbuffer, row, 0, preamble_pattern, preamble_bit_length);
    int32_t start = preamble_start + preamble_bit_length;
    int32_t len = bitbuffer->bits_per_row[row] - start;
    ...
    int32_t fi = bytes.bits_per_row[row] - 8;
    int32_t seen_aa = 0;
    ...
    uint32_t end_byte   = fi;
    uint32_t num_bits   = end_byte - first_byte;
    //uint32_t num_bytes = num_bits/8 / 2;

    bitbuffer_t packet = {0};
    uint32_t fpos = bitbuffer_manchester_decode(&bytes, row, first_byte, &packet, num_bits);
    uint32_t man_errors = num_bits - (fpos - first_byte - 2);
    ...
    uint8_t ds[64] = {0}; // up to 4 ids of at most 10+1 chars
    ...
    case 0x1030: {
        ...
        for (uint32_t i = 0; i < 5; i++) { // order fixed?
