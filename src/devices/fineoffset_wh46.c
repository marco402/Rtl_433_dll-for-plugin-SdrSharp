...
static int32_t fineoffset_wh46_decode(r_device *decoder, bitbuffer_t *bitbuffer, int32_t startPulses, uint16_t package_type)
{
	...
	if (crc != b[19] || sum != b[20]) {
		decoder_logf(decoder, 1, __func__, "Checksum error: %02x %02x", crc, sum);        return DECODE_FAIL_MIC;
	...
	int32_t ext_power = battery_bars == 6;
    ...
    int32_t pm1_raw       = (b[13] << 8) | b[14];
    float pm1         = pm1_raw * 0.1f;
    int32_t pm4_raw       = (b[15] << 8) | b[16];
    float pm4         = pm4_raw * 0.1f;
    int32_t unknown      = (b[17] << 8) | b[18];
