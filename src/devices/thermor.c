...
static int32_t thermor_decode(r_device *decoder, bitbuffer_t *bitbuffer, int32_t startPulses, uint16_t package_type)
{
    ...
    int32_t const wind_dir_degr[] = {157,45,135,67,180,22,112,90,225,337,247,315,202,0,270,292};

    //sync pairing mode
    if (b[0] == 0xff && b[1] == b[2] && b[1] == b[4] && b[1] == b[5] && b[1] == b[6] && b[1] == b[7] && b[1] == b[8] && b[1] == b[10]  ) {
        int32_t new_id = ~b[1] & 0xff;
        /* clang-format off */
        ...
        int32_t have_wdir = 0;
        int32_t have_wspd = 0;
        int32_t rain_rate1 = 0;
        int32_t rain_rate2 = 0;
        int32_t wind_dir_d = 0;
        int32_t wind_coef  = 0;
        ...
        int32_t wind_chk = (~b[6] + ~b[7] + ~b[8]) & 0xff;
        ...
        if (b[8] != 0xff ) {
            ...
            if (wind_speed_raw < 256) {
                wind_ratio = (float)((wind_speed_raw * -0.0001746) + 0.155);
            }
            else {
                wind_ratio = 0.11f;
            }
