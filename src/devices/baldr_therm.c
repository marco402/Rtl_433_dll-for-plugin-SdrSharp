...
*/
static int32_t baldr_therm_decode(r_device *decoder, bitbuffer_t *bitbuffer, int32_t startPulses, uint16_t package_type)
{
    int32_t row = bitbuffer_find_repeated_row(bitbuffer, 8, 64);
    if (row < 0) {
    ...
    if (bitbuffer->bits_per_row[row] > 65) {
