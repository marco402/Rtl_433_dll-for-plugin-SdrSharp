...
static int32_t tpms_gm_decode(r_device *decoder, bitbuffer_t *bitbuffer, int32_t startPulses, uint16_t package_type)
{
    ...
    int32_t pressure_raw    = b[13];
    int32_t temperature_raw = b[14];
    ...
    float pressure_kpa  = (float)(pressure_raw * 2.75);
    float temperature_c = (float)(temperature_raw - 60);

    // Extract bits correctly based on little-endian order
    int32_t bit8 = (flags >> 8) & 1;
    int32_t bit1 = (flags >> 1) & 1;
    int32_t bit0 = (flags >> 0) & 1;

    // Flags bits
    int32_t learn_mode = ((bit8 == 0) && (bit1 == 0) && (bit0 == 0));
    int32_t battery_ok = !((flags >> 5) & 1);
