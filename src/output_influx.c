...
typedef struct {
    ...
    int32_t prev_status;
    int32_t prev_resp_code;
    uint8_t hostname[64];
    uint8_t url[400];
    uint8_t extra_headers[150];
    tls_opts_t tls_opts;
    int32_t databufidxfill;
...
static void influx_client_event(struct mg_connection *nc, int32_t ev, void *ev_data)
{
...
static influx_client_t *influx_client_init(influx_client_t *ctx, uint8_t const *url, uint8_t const *token)
{
...
static uint8_t *influx_sanitize_tag(uint8_t *tag, uint8_t *end)
{
    for (uint8_t *p = tag; *p && p != end; ++p)
...
static uint8_t *mbuf_snprintf(struct mbuf *a, uint8_t const *format, ...)
...
static void mbuf_remove_part(struct mbuf *a, uint8_t *pos, size_t len)
{
...
static void R_API_CALLCONV print_influx_array(data_output_t *output, data_array_t *array, uint8_t const *format, defDeviceToPlugin *ptrDeviceToPlugin)
{
    UNUSED(array);
    UNUSED(format);
...
static void R_API_CALLCONV print_influx_data_escaped(data_output_t *output, data_t *data, uint8_t const *format, defDeviceToPlugin *ptrDeviceToPlugin)
{
    uint8_t str[1000];
    data_print_jsons(data, str, sizeof (str),NULL);
    output->print_string(output, str, format, NULL);
}

static void R_API_CALLCONV print_influx_string_escaped(data_output_t *output, uint8_t const *str, uint8_t const *format, defDeviceToPlugin *ptrDeviceToPlugin)
{
    UNUSED(format);
    influx_client_t *influx = (influx_client_t *)output;
    struct mbuf *databuf = &influx->databufs[influx->databufidxfill];
    size_t size = databuf->size - databuf->len;
    uint8_t *buf = &databuf->buf[databuf->len];
...
static void R_API_CALLCONV print_influx_string(data_output_t *output, uint8_t const *str, uint8_t const *format, defDeviceToPlugin *ptrDeviceToPlugin)
{
    UNUSED(format);
    influx_client_t *influx = (influx_client_t *)output;
    struct mbuf *buf = &influx->databufs[influx->databufidxfill];
...
static void R_API_CALLCONV print_influx_data(data_output_t *output, data_t *data, uint8_t const *format, defDeviceToPlugin *ptrDeviceToPlugin)
{
    UNUSED(format);
    influx_client_t *influx = (influx_client_t *)output;
    uint8_t *str;
...
static void R_API_CALLCONV print_influx_double(data_output_t *output, double data, uint8_t const *format, defDeviceToPlugin *ptrDeviceToPlugin)
{
    UNUSED(format);
    influx_client_t *influx = (influx_client_t *)output;
    struct mbuf *buf = &influx->databufs[influx->databufidxfill];
...
static void R_API_CALLCONV print_influx_int(data_output_t *output, int32_t data, uint8_t const *format, defDeviceToPlugin *ptrDeviceToPlugin)
{
    UNUSED(format);
    influx_client_t *influx = (influx_client_t *)output;
    struct mbuf *buf = &influx->databufs[influx->databufidxfill];
...
//    uint8_t *dot = strchr(influx->hostname, '.');
//    if (dot)
//        *dot = '\0';
