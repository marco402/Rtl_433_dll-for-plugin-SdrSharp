...
int mg_if_poll(struct mg_connection *nc, double now)
{
     ...
     * drain them. */
        int recved = 0;
        do {
            if (nc->flags & (MG_F_WANT_READ | MG_F_WANT_WRITE))
                break;
            if (recv_avail_size(nc, MG_TCP_IO_SIZE) <= 0)
                break;
...
int mg_socketpair(sock_t sp[2], int sock_type)
{
    ...
    if ((sock = socket(AF_INET, sock_type, 0)) == INVALID_SOCKET) {
    ...
    else if ((sp[1] = (sock_type == SOCK_DGRAM ? sock : mg_socketpair_accept(sock, &sa, len))) ==
             INVALID_SOCKET) {
        // abort
    }
    else {
        ...
        if (sock_type == SOCK_STREAM)
            mg_socketpair_close(&sock);
        ret = 1;
    }

    if (!ret) {
        if (sp[0] != INVALID_SOCKET)
            mg_socketpair_close(&sp[0]);
        if (sp[1] != INVALID_SOCKET)
            mg_socketpair_close(&sp[1]);
        if (sock != INVALID_SOCKET)
            mg_socketpair_close(&sock);
    }
...
static void socks_if_relay(struct mg_connection *s)
{
    ...
    if (s->recv_mbuf.len > 0)
        mg_if_can_recv_cb(d->c);
...
int mg_socks_if_tcp_recv(struct mg_connection *c, void *buf, size_t len)
{
    ...
    if (d->s == NULL)
        return -1;
    if (len > d->s->recv_mbuf.len)
        len = d->s->recv_mbuf.len;
    if (len > 0) {
...
const struct mg_iface_vtable mg_socks_iface_vtable = {
        mg_socks_if_init,
        mg_socks_if_free,
        mg_socks_if_add_conn,
        mg_socks_if_remove_conn,
        mg_socks_if_poll,
        mg_socks_if_listen_tcp,
        mg_socks_if_listen_udp,
        mg_socks_if_connect_tcp,
        mg_socks_if_connect_udp,
        mg_socks_if_tcp_send,
        mg_socks_if_udp_send,
        mg_socks_if_tcp_recv,
        mg_socks_if_udp_recv,
        mg_socks_if_create_conn,
        mg_socks_if_destroy_conn,
        mg_socks_if_sock_set,
...
int mg_ssl_if_write(struct mg_connection *nc, const void *buf, size_t len)
{
   ...
   * be removed. */
    size_t l = len;
    if (ctx->saved_len > 0 && ctx->saved_len < l)
        l = ctx->saved_len;
...
enum mg_http_proto_data_type { DATA_NONE,
    DATA_FILE,
    DATA_PUT };
...
out:
    if (conn_addr != NULL && conn_addr != conn_addr_buf)
        MG_FREE(conn_addr);
    return nc;
}
        ...
        size_t *data_len)
{
    ...
    if (buf == NULL || buf_len <= 0)
        return 0;
    if ((shl = mg_http_get_request_len(buf, buf_len)) <= 0)
        return 0;
    hl = shl;
...
//#if MG_ENABLE_THREADS //RTL_433.dll
        ...
        const void *buf, int num_bytes, int ev)
{
    ...
    if (tag == NULL)
        return; /* Don't log MG_EV_TIMER, etc */
...
uint32_t mg_coap_compose(struct mg_coap_message *cm, struct mbuf *io)
{
    ...
    prev_io_len = io->len;
    if (mbuf_append(io, NULL, packet_size) == 0)
...
static void mg_socks5_handshake(struct mg_connection *c)
{
    ...
    if (r->buf[0] != MG_SOCKS_VERSION) {
        ...
        for (i = 2; i < r->buf[1] + 2; i++) {
            ...
            if (r->buf[i] == MG_SOCKS_HANDSHAKE_NOAUTH)
                reply[1] = r->buf[i];
        }
...
static void serv_ev_handler(struct mg_connection *c, int ev, void *ev_data)
{
    if (ev == MG_EV_CLOSE) {
        ...
        int res = *(int *)ev_data;
        if (res != 0)
            LOG(LL_ERROR, ("connect error: %d", res));
    }
}
...
static void mg_socks5_handle_request(struct mg_connection *c)
{
    ...
    if (ver != MG_SOCKS_VERSION || cmd != MG_SOCKS_CMD_CONNECT) {
        ...
        addr_len = 4;
        if (r->len < (size_t)addr_len + 6)
            return; /* return if not buffered */
...
static void socks_handler(struct mg_connection *c, int ev, void *ev_data)
{
    if (ev == MG_EV_RECV) {
        if (!(c->flags & MG_SOCKS_HANDSHAKE_DONE))
            mg_socks5_handshake(c);
        if (c->flags & MG_SOCKS_HANDSHAKE_DONE &&
                !(c->flags & MG_SOCKS_CONNECT_DONE)) {
            mg_socks5_handle_request(c);
        }
        if (c->flags & MG_SOCKS_CONNECT_DONE)
            relay_data(c);
    }
    else if (ev == MG_EV_CLOSE) {
        disband(c);
        ...
        int proto)
{
    ...
    if (sock < 0)
        return sock;
#if MG_ENABLE_SSL && MG_SSL_IF == MG_SSL_IF_SIMPLELINK
    if ((r = sl_set_ssl_opts(sock, nc)) < 0)
        goto clean;
#endif
    if ((r = sl_Bind(sock, &sa->sa, sa_len)) < 0)
        goto clean;
    if (type != SOCK_DGRAM) {
        if ((r = sl_Listen(sock, SOMAXCONN)) < 0)
            goto clean;
    }
    mg_set_non_blocking_mode(sock);
clean:
...
int sl_set_ssl_opts(int sock, struct mg_connection *nc)
{
     ...
     * so we ignore the error. */
        if (err != 0 && err != SL_ERROR_BSD_ENOPROTOOPT)
            return err;
    }
    return 0;
}
