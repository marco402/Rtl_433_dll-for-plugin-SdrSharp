...
#ifndef DLL_RTL_433
void set_freq_correction(r_cfg_t *cfg, int32_t freq_correction)
{
...
#undef DECL
    ...
    for (uint32_t i = 0; i < cfg->num_r_devices; i++) {
...
void register_protocol(r_cfg_t *cfg, r_device *r_dev, bool _sourceIsFile, uint8_t *arg)
{
    ...
    r_device *p;
    if (r_dev->create_fn) {
        p = r_dev->create_fn(arg,r_dev->protocol_num);
    }
    else {
        if (arg && *arg) {
...
void calc_rssi_snr(r_cfg_t *cfg, pulse_data_t *pulse_data)
{
    ...
    pulse_data->centerfreq_hz = (float)(cfg->center_frequency);
...
uint8_t *time_pos_str(r_cfg_t *cfg, uint32_t samples_ago, uint8_t *buf)
{
    if (cfg->report_time == REPORT_TIME_SAMPLES) {
        double s_per_sample = 1.0f / cfg->samp_rate;
        return sample_pos_str((float)(cfg->demod->sample_file_pos - samples_ago * s_per_sample), buf);
    }
    else {
        struct timeval ago   = cfg->demod->now;
        double us_per_sample = 1e6 / cfg->samp_rate;
        uint32_t usecs_ago   = (uint32_t)(samples_ago * us_per_sample);
        while (ago.tv_usec < (int32_t)usecs_ago) {
            ago.tv_sec --;
            ago.tv_usec += 1000000;
        }
        ago.tv_usec -= usecs_ago;

        uint8_t const *format = NULL;
...
uint8_t const **well_known_output_fields(r_cfg_t *cfg)
{
    list_t field_list = {0};
    ...
    return (uint8_t const **)field_list.elems;
}

/** Convert CSV keys according to selected conversion mode. Replacement is static but in-place. */
static uint8_t const **convert_csv_fields(r_cfg_t *cfg, uint8_t const **fields)
{
    if (cfg->conversion_mode == CONVERT_SI) {
        for (uint8_t const **p = fields; *p; ++p) {
...
uint8_t const **determine_csv_fields(r_cfg_t *cfg, uint8_t const *const *well_known, int32_t *num_fields)
{
    list_t field_list = {0};
    ...
    convert_csv_fields(cfg, (uint8_t const **)field_list.elems);

    if (num_fields)
        *num_fields = (int32_t)field_list.len;
    return (uint8_t const **)field_list.elems;
}

int32_t run_ook_demods(list_t *r_devs, pulse_data_t *pulse_data, int32_t startPulses, uint16_t package_type)
{
    int32_t p_events = 0;

    uint32_t next_priority = 0; // next smallest on each loop through decoders
    // run all decoders of each priority, stop if an event is produced
    for (uint32_t priority = 0; !p_events && priority < UINT_MAX; priority = next_priority) {
        ...
        for (void **iter = r_devs->elems; iter && *iter; ++iter) {
            ...
            case OOK_PULSE_PCM:
                // case OOK_PULSE_RZ:
                p_events += pulse_slicer_pcm(pulse_data, r_dev, startPulses,  package_type);
                break;
            case OOK_PULSE_PPM:
                p_events += pulse_slicer_ppm(pulse_data, r_dev, startPulses,  package_type);
                break;
            case OOK_PULSE_PWM:
                p_events += pulse_slicer_pwm(pulse_data, r_dev, startPulses,  package_type);
                break;
            case OOK_PULSE_MANCHESTER_ZEROBIT:
                p_events += pulse_slicer_manchester_zerobit(pulse_data, r_dev, startPulses,  package_type);
                break;
            case OOK_PULSE_PIWM_RAW:
                p_events += pulse_slicer_piwm_raw(pulse_data, r_dev, startPulses,  package_type);
                break;
            case OOK_PULSE_PIWM_DC:
                p_events += pulse_slicer_piwm_dc(pulse_data, r_dev, startPulses,  package_type);
                break;
            case OOK_PULSE_DMC:
                p_events += pulse_slicer_dmc(pulse_data, r_dev, startPulses,  package_type);
                break;
            case OOK_PULSE_PWM_OSV1:
                p_events += pulse_slicer_osv1(pulse_data, r_dev, startPulses,  package_type);
                break;
            case OOK_PULSE_NRZS:
                p_events += pulse_slicer_nrzs(pulse_data, r_dev, startPulses,  package_type);
                break;
            // FSK decoders
            case FSK_PULSE_PCM:
            case FSK_PULSE_PWM:
...
int32_t run_fsk_demods(list_t *r_devs, pulse_data_t *fsk_pulse_data, int32_t startPulses, uint16_t package_type)
{
    int32_t p_events = 0;
    ...
    for (uint32_t priority = 0; !p_events && priority < UINT_MAX; priority = next_priority) {
        ...
        for (void **iter = r_devs->elems; iter && *iter; ++iter) {
            ...
            case OOK_PULSE_NRZS:
                break;
            case FSK_PULSE_PCM:
                p_events += pulse_slicer_pcm(fsk_pulse_data, r_dev, startPulses,  package_type);
                break;
            case FSK_PULSE_PWM:
                p_events += pulse_slicer_pwm(fsk_pulse_data, r_dev, startPulses,  package_type);
                break;
            case FSK_PULSE_MANCHESTER_ZEROBIT:
                p_events += pulse_slicer_manchester_zerobit(fsk_pulse_data, r_dev, startPulses,  package_type);
                break;
            default:
                fprintf(stderr, "Unknown modulation %u in protocol!\n", r_dev->modulation);
            }
...
static void log_handler(log_level_t level, uint8_t const *src, uint8_t const *msg, void *userdata)
{
    r_cfg_t *cfg = userdata;

    if (cfg->verbosity < (int32_t)level) {
        return;
    }
    /* clang-format off */
    ...
    defDeviceToPlugin deviceToPlugin;
    ...
    for (size_t i = 0; i < cfg->output_handler.len; ++i) { // list might contain NULLs
        ...
        if (output && output->log_level >= (int32_t)level) {
            data_output_print(output, data, ptrDeviceToPlugin);
        }
    }
    data_free(data);
}
...
/** Pass the data structure to all output handlers. Frees data afterwards. */
void log_device_handler(r_device *r_dev, int32_t level, data_t *data)
{
    ...
    for (size_t i = 0; i < cfg->output_handler.len; ++i) { // list might contain NULLs
        ...
        if (output && output->log_level >= level) {
            data_output_print(output, data, NULL);
        }
    }
    data_free(data);
}
...
/** Pass the data structure to all output handlers. Frees data afterwards. */
void data_acquired_handler(r_device *r_dev, data_t *data, defDeviceToPlugin *ptrDeviceToPlugin)
{
    //r_deviceToPlugin *dp = (r_deviceToPlugin *)infosToPlugin;
    r_cfg_t *cfg = r_dev->output_ctx;
//#if SYNCHROGRAPH
    ...
    if (cfg->conversion_mode == CONVERT_SI) {
        for (data_t *d = data; d; d = d->next) {
            ...
            if ((d->type == DATA_DOUBLE) && str_endswith(d->key, "_F")) {
                d->value.v_dbl  = fahrenheit2celsius((float)d->value.v_dbl);
                uint8_t *new_label = str_replace(d->key, "_F", "_C");
                free(d->key);
                d->key = new_label;
                uint8_t *pos;
            ...
            // Convert double type fields ending in _mi_h to _km_h
                ...
                d->value.v_dbl  = mph2kmph((float)(d->value.v_dbl));
                uint8_t *new_label = str_replace(d->key, "_mi_h", "_km_h");
                free(d->key);
                d->key                 = new_label;
                uint8_t *new_format_label = str_replace(d->format, "mi/h", "km/h");
                free(d->format);
            ...
            // Convert double type fields ending in _in to _mm
                ...
                d->value.v_dbl  = inch2mm((float)(d->value.v_dbl));
                uint8_t *new_label = str_replace(d->key, "_in", "_mm");
                free(d->key);
                d->key                 = new_label;
                uint8_t *new_format_label = str_replace(d->format, "in", "mm");
                free(d->format);
                d->format = new_format_label;
            }
            // Convert double type fields ending in _in_h to _mm_h
                ...
                d->value.v_dbl  = inch2mm((float)(d->value.v_dbl));
                uint8_t *new_label = str_replace(d->key, "_in_h", "_mm_h");
                free(d->key);
                d->key                 = new_label;
                uint8_t *new_format_label = str_replace(d->format, "in/h", "mm/h");
                free(d->format);
                d->format = new_format_label;
            }
            // Convert double type fields ending in _inHg to _hPa
            else if ((d->type == DATA_DOUBLE) && str_endswith(d->key, "_inHg")) {
                d->value.v_dbl  = inhg2hpa((float)(d->value.v_dbl));
                uint8_t *new_label = str_replace(d->key, "_inHg", "_hPa");
                free(d->key);
                d->key                 = new_label;
                uint8_t *new_format_label = str_replace(d->format, "inHg", "hPa");
                free(d->format);
                d->format = new_format_label;
            }
            // Convert double type fields ending in _PSI to _kPa
            else if ((d->type == DATA_DOUBLE) && str_endswith(d->key, "_PSI")) {
                d->value.v_dbl  = psi2kpa((float)(d->value.v_dbl));
                uint8_t *new_label = str_replace(d->key, "_PSI", "_kPa");
                free(d->key);
                d->key                 = new_label;
                uint8_t *new_format_label = str_replace(d->format, "PSI", "kPa");
                free(d->format);
                d->format = new_format_label;
            }
        }
    ...
    if (cfg->conversion_mode == CONVERT_CUSTOMARY) {
        for (data_t *d = data; d; d = d->next) {
            ...
            if ((d->type == DATA_DOUBLE) && str_endswith(d->key, "_C")) {
                d->value.v_dbl  = celsius2fahrenheit((float)(d->value.v_dbl));
                uint8_t *new_label = str_replace(d->key, "_C", "_F");
                free(d->key);
                d->key = new_label;
                uint8_t *pos;
            ...
            // Convert double type fields ending in _km_h to _mi_h
                ...
                d->value.v_dbl  = kmph2mph((float)(d->value.v_dbl));
                uint8_t *new_label = str_replace(d->key, "_km_h", "_mi_h");
                free(d->key);
                d->key                 = new_label;
                uint8_t *new_format_label = str_replace(d->format, "km/h", "mi/h");
                free(d->format);
                d->format = new_format_label;
            }
            // Convert double type fields ending in _mm to _inch
            else if ((d->type == DATA_DOUBLE) && str_endswith(d->key, "_mm")) {
                d->value.v_dbl  = mm2inch((float)(d->value.v_dbl));
                uint8_t *new_label = str_replace(d->key, "_mm", "_in");
                free(d->key);
                d->key                 = new_label;
                uint8_t *new_format_label = str_replace(d->format, "mm", "in");
                free(d->format);
                d->format = new_format_label;
            }
            // Convert double type fields ending in _mm_h to _in_h
                ...
                d->value.v_dbl  = mm2inch((float)(d->value.v_dbl));
                uint8_t *new_label = str_replace(d->key, "_mm_h", "_in_h");
                free(d->key);
                d->key                 = new_label;
                uint8_t *new_format_label = str_replace(d->format, "mm/h", "in/h");
                free(d->format);
                d->format = new_format_label;
            }
            // Convert double type fields ending in _hPa to _inHg
            else if ((d->type == DATA_DOUBLE) && str_endswith(d->key, "_hPa")) {
                d->value.v_dbl  =hpa2inhg( (float)(d->value.v_dbl));
                uint8_t *new_label = str_replace(d->key, "_hPa", "_inHg");
                free(d->key);
                d->key                 = new_label;
                uint8_t *new_format_label = str_replace(d->format, "hPa", "inHg");
                free(d->format);
                d->format = new_format_label;
            }
            // Convert double type fields ending in _kPa to _PSI
            else if ((d->type == DATA_DOUBLE) && str_endswith(d->key, "_kPa")) {
                d->value.v_dbl  = kpa2psi((float)(d->value.v_dbl));
                uint8_t *new_label = str_replace(d->key, "_kPa", "_PSI");
                free(d->key);
                d->key                 = new_label;
                uint8_t *new_format_label = str_replace(d->format, "kPa", "PSI");
                free(d->format);
                d->format = new_format_label;
            }
        }
...
data_t *create_report_data(r_cfg_t *cfg, int32_t level)
{
    list_t *r_devs = &cfg->demod->r_devs;
    data_t *data;
    ...
    uint8_t since_str[LOCAL_TIME_BUFLEN];
    ...
    data = data_make(
            ...
            "stats", "", DATA_ARRAY, data_array((int32_t)dev_data_list.len, DATA_DATA, dev_data_list.elems),
            NULL);
...
static int32_t lvlarg_param(uint8_t **param, int32_t default_verb)
{
    if (!param || !*param) {
        return default_verb;
    }
    ...
    while (*p == ' ' || *p == '\t')
        p++;
    uint8_t *endptr;
    int32_t val = strtol(p, &endptr, 10);
    if (p == endptr) {
...
static FILE *fopen_output(uint8_t const *param)
{
    if (!param || !*param) {
...
//void add_json_output(r_cfg_t *cfg, uint8_t *param)
//{
//    int32_t log_level = lvlarg_param(&param, 0);
//    list_push(&cfg->output_handler, data_output_json_create(log_level, fopen_output(param)));
//}

//void add_csv_output(r_cfg_t *cfg, uint8_t *param)
//{
//    int32_t log_level = lvlarg_param(&param, 0);
//    list_push(&cfg->output_handler, data_output_csv_create(log_level, fopen_output(param)));
//}

void start_outputs(r_cfg_t *cfg, uint8_t const *const *well_known)
{
    int32_t num_output_fields;
    uint8_t const **output_fields = determine_csv_fields(cfg, well_known, &num_output_fields);
...
void add_log_output(r_cfg_t *cfg, uint8_t *param)
{
    int32_t log_level = lvlarg_param(&param, LOG_TRACE);
    list_push(&cfg->output_handler, data_output_log_create(log_level, fopen_output(param)));
}

void add_kv_output(r_cfg_t *cfg, uint8_t *param)
{
    int32_t log_level = lvlarg_param(&param, LOG_TRACE);
    list_push(&cfg->output_handler, data_output_kv_create(log_level, fopen_output(param)));
}

//void add_mqtt_output(r_cfg_t *cfg, uint8_t *param)
//{
//    list_push(&cfg->output_handler, data_output_mqtt_create(get_mgr(cfg), param, cfg->dev_query));
//}

//void add_influx_output(r_cfg_t *cfg, uint8_t *param)
//{
//    list_push(&cfg->output_handler, data_output_influx_create(get_mgr(cfg), param));
//}

//void add_syslog_output(r_cfg_t *cfg, uint8_t *param)
//{
//    int32_t log_level     = lvlarg_param(&param, LOG_WARNING);
//    uint8_t const *host  = "localhost";
//    uint8_t const *port  = "514";
//    uint8_t const *extra = hostport_param(param, &host, &port);
//    if (extra && *extra) {
...
//}
#ifndef DLL_RTL_433
void add_http_output(r_cfg_t *cfg, uint8_t *param)
{
    // Note: no log_level, the HTTP-API consumes all log levels.
    uint8_t const *host  = "0.0.0.0";
    uint8_t const *port  = "8433";
    uint8_t const *extra = hostport_param(param, &host, &port);
    if (extra && *extra) {
...
void add_sr_dumper(r_cfg_t *cfg, uint8_t const *spec, int32_t overwrite)
{
    // create channels
...
void reopen_dumpers(struct r_cfg *cfg)
{
	...
	for (void **iter = cfg->demod->dumper.elems; iter && *iter; ++iter) {
		...
		if (dumper->file && (dumper->file != stdout)) {
			...
			int32_t ret = fstat(fileno(dumper->file), &old_st);
			if (ret) {
...
void add_dumper(r_cfg_t *cfg, uint8_t const *spec, int32_t overwrite)
{
    ...
    if (strcmp(dumper->path, "-") == 0) { /* Write samples to stdout */
        ...
        if (_access(dumper->path, F_OK) == 0 && !overwrite) {
...
void add_infile(r_cfg_t *cfg, uint8_t *in_file)
{
